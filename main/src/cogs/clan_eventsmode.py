...
from event_list import EnumEventList
...
class EventsMode(BaseCog):
    ...
    @commands.cooldown(1, 60, commands.BucketType.member)  # the command can only be used once in 60 seconds
    async def event_request(self, interaction: discord.Interaction, event_list: Option(str, 'chose item', choices=EnumEventList.list(), required=True)):
        button_accept = Button(style=discord.ButtonStyle.secondary, label='Взяти', emoji='❕')
        button_end = Button(style=discord.ButtonStyle.secondary, label='Здати', emoji='❗')
        button_decline = Button(style=discord.ButtonStyle.secondary, label='Отказ', emoji='❌')
                    ...
                    for member in channel.members:
                        if member.id == interaction.user.id:
                            ...
                            request_msg = await self.clan_event_chat.send(
                                embed=RequestEmbed(clan_name=channel.name, event_name=event_list, interaction=interaction,
                                                   members_on_voice=sum([1 for member in channel.members]), clan_control_role=self.clan_control_role.mention).embed,
                                view=view)
                            events_system.request_create(message_id=request_msg.id, clan_name=channel.name, event_name=event_list, member_id=member.id)
                            ...
                            async def accept_callback(ctx):
                                ...
                                await member.send(embed=DefaultEmbed(f'***```{ctx.user.name}, принял запрос ивента;\nОжидайте в ближайшее время.```***'))
                            ...
                            async def decline_callback(ctx):
                                ...
                                await member.send(embed=DefaultEmbed(f'***```{ctx.user.name}, отклонил запрос на проведение ивента```***'))
                                await request_msg.edit(
                            ...
                            async def pass_callback(ctx):
                                ...
                                if check_request_id is None:
                                    return await ctx.send(f'{ctx.user.name}, это не ваш ивент.')
    ...
    async def events_list(self, ctx):
        ...
        for member in members:
            ...
            total_time = time.gmtime(member["waisting_time"])
            description += f'{counter} - {get_member} - {str(member["event_num"])} - {str(time.strftime("%H:%M:%S", total_time))} - <t:{member["add_time"]}>' + '\n'
            counter += 1
