...
class ClanSystem(DatabaseSystem):
    ...
    def clan_profile(self, clan_role_id):
        return self.clan_member_details_collection.find({'clan_role_id': clan_role_id}, {'_id': 0, 'member_id': 1, 'member_online': 1}).sort('member_online',
                                                                                                                                             -1)
    ...
    def clan_pay(self, clan_role_id: int, amount: int):
        self.clan_collection.update_one({'clan_role_id': clan_role_id}, {'$inc': {'clan_cash': -amount}})
        return True
    ...
    def add_zam_member_to_clan(self, clan_role_id: int, member_id: int):
        self.clan_zam_collection.update_one({'clan_role_id': clan_role_id}, {'$push': {'zam_member_id': {'member_id': member_id}}})
    ...
    def remove_zam_member_from_clan(self, clan_role_id: int, member_id: int):
        self.clan_zam_collection.update_one({'clan_role_id': clan_role_id}, {'$pull': {'zam_member_id': {'member_id': member_id}}})
        self.clan_collection.update_one({'clan_role_id': clan_role_id}, {'$inc': {"zam_slot": 1}})
        return True

    def buy_slot_clan_zam(self, leader_id: int, how: int):
        self.clan_collection.update_one({'leader_id': leader_id}, {'$inc': {"zam_slot": how}})
        return True

    def buy_clan_slot(self, leader_id: int, how: int):
        self.clan_collection.update_one({'leader_id': leader_id}, {'$inc': {"clan_member_slot": how}})
        return True

    def clan_leave(self, member_id: int) -> tuple:
        res_role_id = self.clan_member_collection.find_one({'member_id': member_id})
        res = self.clan_collection.find_one({'clan_role_id': res_role_id["clan_role_id"]})
        ...
        self.clan_collection.update_one({'clan_role_id': res_role_id["clan_role_id"]}, {"$inc": {'clan_member_number': -1}})
        self.clan_member_collection.delete_one({'member_id': member_id})
        self.clan_member_details_collection.delete_one({'member_id': member_id})

        return res['clan_name'], res_role_id['clan_role_id']

    def check_author_on_clan(self, member_id: int):
        if self.clan_collection.find_one({'leader_id': member_id}):
            return member_id
        return False

    def check_member_for_a_zam_by_member_id(self, member_id: int):
        if self.clan_zam_collection.find_one({'zam_member_id': {"member_id": member_id}}):
            return member_id
        return False
